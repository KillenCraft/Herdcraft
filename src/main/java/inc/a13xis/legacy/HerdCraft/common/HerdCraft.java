...
import inc.a13xis.legacy.HerdCraft.entity.ai.*;
...
@Mod(modid = "herdCraft", name = "HerdCraft", version = "L1.0", guiFactory = "inc.a13xis.legacy.HerdCraft.common.guiconfig.HerdConfigGUIFactory", acceptedMinecraftVersions = "[1.9,1.11.2]")
public class HerdCraft 
{
	...
	@SubscribeEvent
	 public void onConfigChanged(ConfigChangedEvent.OnConfigChangedEvent eventArgs) {
        if(eventArgs.getModID().equals("herdCraft"))
            refreshConfig();
    }
	...
	public void load(FMLInitializationEvent event)
	{
		...
		MinecraftForge.EVENT_BUS.register(herdCollectionObj);
	}
	...
	private void handleZombie(EntityCreature animal)
	{
		...
		animal.tasks.addTask(2, new EntityAIHerdAttackMelee(animal, EntityPlayer.class, 1.0D, false,(EXPLICIT_ZOMBIE_HORDING?EntityZombie.class:null)));
        animal.tasks.addTask(3, new EntityAIHerdAttackMelee(animal, EntityVillager.class, 1.0D, true,(EXPLICIT_ZOMBIE_HORDING?EntityZombie.class:null)));
		...
		while (i.hasNext()) 
		{
			...
			{ }
			if (aiTask.action instanceof EntityAIAttackMelee) {
				i.remove();
			}     
		}
	}
	...
	private void handleSkeleton(EntityCreature animal) {
		...
		while (i.hasNext()) 
		{
			...
			{ }
			if (aiTask.action instanceof EntityAIAttackRangedBow) {
				i.remove();
				hadArrow = true;
			}     
		}
		if (hadArrow)
		{
			animal.tasks.addTask(4, new EntityAIHerdAttackRangeBow((IRangedAttackMob) animal, 1.0D, 20, 60, 15.0F,SKELETON_CAUTION_ANGLE, rand.nextBoolean(), SKELETON_FEAR_MELEE));
    ...
    public void onLivingSpawnEvent(LivingSpawnEvent e)
    {
    	...
    	if (e.getEntity().getClass() != EntityZombie.class) return;
    	EntityZombie animal = (EntityZombie)e.getEntity();
	...
	private int hordeAdjustments(EntityZombie entity) {
		...
		int y;
		for(y = (int)entity.posY; y < world.getActualHeight() && world.isAirBlock(new BlockPos((int)entity.posX, y, (int)entity.posZ)); y++);//stops at first non-air block
    	...
    	{// Above ground
			...
			Biome biome = world.getBiomeGenForCoords(new BlockPos((int)entity.posX,0,(int)entity.posZ));
	    	...
	    	float hills = biome.getHeightVariation();
	    	if (hills < 0.2)
	    	{
